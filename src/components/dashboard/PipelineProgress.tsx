  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-secondary/10 shadow-[0_20px_60px_-15px_rgba(0,49,120,0.14)]">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="relative flex size-2.5">
            {isRunning && (
              <span className="absolute inline-flex size-full animate-ping rounded-full opacity-60" style={{ background: "#70d8c8" }} />
            )}
            <span
              className={cn(
                ...
                !isError && !isReportDone && !isRunning && "bg-white/20",
              )}
              style={(!isError && !isReportDone && isRunning) ? { background: "#70d8c8" } : undefined}
            />
          </span>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/35">Processing</p>
            <h2 className="text-base font-bold text-white leading-tight">Research Pipeline</h2>
          </div>
        </div>
        ...
        <span
          className={cn(
            ...
            isReportDone && "bg-tertiary-fixed-dim/20 text-tertiary-fixed-dim",
            isReportRunning && "bg-white/10 text-white/70",
            isRunning && !isReportRunning && "bg-white/10 text-white/70",
            !isError && !isReportDone && !isRunning && "bg-white/8 text-white/30",
          )}
        >
          {statusLabel}
        </span>
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={isCurrent ? { background: "#70d8c8", color: "#001228", boxShadow: "0 10px 25px -5px rgba(112, 216, 200, 0.4)" } : undefined}
                  >
                    {isCurrent ? (
      ...
      {events.length > 0 && (
        <div
          ref={logRef}
          className="max-h-52 overflow-y-auto scroll-smooth px-5 py-3 font-mono bg-white"
          ...
          <div className="space-y-1">
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isLast && isRunning && "ring-1 ring-inset ring-black/4",
                  )}
                >
                  {/* Left icon */}
                  ...
                  {/* Timestamp */}
                  <span className="shrink-0 text-[10px] text-secondary/45 tabular-nums mt-px">
                  ...
                  <span
                    className={cn(
                      "shrink-0 self-start mt-px rounded px-1 py-px text-[9px] font-bold uppercase tracking-wide leading-tight",
                      tag.badgeClass,
                    )}
                  >
                    {tag.label}
            ...
            {isRunning && (
              <div className="flex items-center gap-2 px-2 py-0.5">
                ...
                <span className="h-[11px] w-px animate-pulse bg-primary/60" />
              </div>
            )}
          </div>
        </div>
      ...
      {events.length === 0 && !isRunning && (
        <div className="flex items-center justify-center py-5 text-xs text-secondary/50 bg-white">
